...
class LivevideoModel {
	...
	function syncRoomList()
	{
		...
		while (true) {
			...
			foreach ($result['room_info'] as $room) {
				...
				$updateData = array('name' => (string) $room['name'], 'cover_img' => (string) $room['cover_img'], 'live_status' => (int) $room['live_status'], 'start_time' => (int) $room['start_time'], 'end_time' => (int) $room['end_time'], 'anchor_name' => (string) $room['anchor_name'], 'anchor_img' => (string) $room['anchor_img'], 'goods' => json_encode($room['goods']));
	...
	function syncLiveReplay($room_id)
	{
		...
		if($res->errcode == 0) {
			...
			M('lionfish_comshop_wxlive')->where( array('roomid' => $room_id ) )->save($updateData);
			return $live_replay;
		} else {
			// 代表未创建直播房间
			return '';
