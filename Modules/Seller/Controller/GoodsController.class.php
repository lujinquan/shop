		...
		if (!(empty($total))) {
			...
			foreach ($list as $key => &$value ) {
                ...
                if(I('get.export') > 0) {// 商品列表到处excel时要导出规格
                    ...
                    $goodsCommon = M('lionfish_comshop_good_common')->where('goods_id = "'.$value['id'].'"')->field('is_spike_buy,is_new_buy,one_limit_count,total_limit_count,begin_time,end_time')->find();
                    ...
                    if($goodsPin){
                        $value['pin_begin_time'] = $goodsPin['begin_time'];
                        $value['pin_end_time'] = $goodsPin['end_time'];
                    }
