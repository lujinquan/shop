...
//dump($need_data['order_ids']);exit;
	...
	public function update_delivery_date()
	{
	    ...
	    //show_json(1, array('url' => $_SERVER['HTTP_REFERER']));
		show_json(0,  array('message' => '日期格式错误' ) );
	}
	...
	public function oprefund_do()
	{
		...
		$has_refud_money = M('lionfish_comshop_order_goods')->where( array('order_id' => $id ) )->sum('has_refund_money');
		
		$has_refund_quantity = M('lionfish_comshop_order_goods')->where( array('order_id' => $id ) )->sum('has_refund_quantity');
			...
			else if( $refund_money <= 0 )
			{
				...
				if( $order_info['order_status_id'] != 7 )
				{
					...
					if( $res['code'] == 0 )
					{
						...
						D('Seller/Commonorder')->refund_one_order( $id );
	...
	public function oprefund_doform()
	{
		...
		if (IS_POST) {
			...
			if($cansub == 1)
			{
				...
				$order_refund = M('lionfish_comshop_order_refund')->field('ref_money,real_refund_quantity')->where( array('ref_id' => $ref_id ) )->find();
