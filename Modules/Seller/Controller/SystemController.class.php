...
class SystemController extends \Think\Controller {
	...
	public function auth_submission()
	{
		...
		$auth['url'] = trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($url, '/')));
		$query            = base64_encode(json_encode($auth));
		$auth['auth_url']         = HTTP_X_FOR . 'as.shiziyu8888.com/upgrade/index/auth?__auth=' . $query;
		$this->data = $auth;
	...
	{
		define('HTTP_X_FOR', (isset($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] == '443') ? 'https://' : 'http://');		
		$auth_url =HTTP_X_FOR ."as.shiziyu8888.com/upgrade/index";
